    ...
    switch (c.toLowerCase()) {
      case 'help':
        push(
          ...
          '',
          'Tip: use ↑ / ↓ to navigate history. Ctrl+L to clear.',
        )
        break
      ...
      case 'pwd':
        push('/home/vansh/portfolio')
        break

      case 'ls':
        push('about.ts  skills.json  experience.ts  contact.ts  resume.pdf')
        break
      ...
      // Easter eggs
      case 'sudo':
        push('sudo: permission denied. (nice try though 😄)')
        break

      case 'rm':
        push('rm: permission denied. this portfolio is staying right here.')
        break
      ...
      case 'ssh':
        push("ssh: connect to host vansh.dev port 22: no route to host (he's a frontend dev 😅)")
        break
      ...
      case 'pokemon': case 'lucario':
        push(
          'Lucario — Aura Pokémon  #448',
          'Type: Fighting / Steel · Ability: Inner Focus',
          'Favorite Pokémon. Will talk about this at length. Don\'t start him. 🐾',
        )
        break
      ...
      case 'uname':
        push('portfolio-v1 React19 arm64 TypeScript5.7')
        break

      case 'open': case 'xdg-open':
        push('Try the tabs above instead.')
        break

      default:
        push(`${c}: command not found. type 'help' to see available commands.`)
    }
  }
